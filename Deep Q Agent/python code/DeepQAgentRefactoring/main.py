...
if __name__ == '__main__':
    ...
    parser.add_argument('-max_mem', type=int, default=50000, help='Maximum size for memory replay buffer')
    ...
    agent = agent_(gamma=0.99, epsilon=1.0, lr=0.0001, input_dims=(env.observation_space.shape), n_actions=env.action_space.n, mem_size=50000,
                     eps_min=0.1, batch_size=32, replace=1000, eps_dec=1e-5, chkpt_dir='models/', algo= 'DuelingDDQNAgent', env_name='PongNoFrameskip-v4')
    if load_checkpoint:
        agent.load_models()
        
    fname = agent.algo + '_' + agent.env_name + 'lr' + str(agent.lr) + '_' + '_' + str(n_games) + 'games'
