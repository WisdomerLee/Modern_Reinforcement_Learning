...
class Agent():
    ...
    def learn(self, state, reward, state_, done):
        ...
        state = T.tensor([state_], dtype=T.float).to(self.actor_critic.device)
        reward = T.tensor(reward, dtype=T.float).to(self.actor_critic.device)
        _, critic_value = self.actor_critic.forward(state)
        _, critic_value = self.actor_critic.forward(state_)
...
if __name__ == '__main__':
    ...
    n_games =3000
    fname = 'ACTORCRITIC_' + 'lunar_lander_' + str(agent.fc1_dims) + '_fc1_dims_' + str(agent.fc2_dims) + '_fc2_dims_lr' + str(agent.lr) + '_' + str(n_games) + 'games'
